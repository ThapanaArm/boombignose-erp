...
import type { FieldAgent, AgentStatus } from "@/lib/repo-agents";
...
const EMPTY = { name: "", phone: "", zone: "", status: "available" as AgentStatus };

export default function AgentsPage() {
  const [items, setItems] = useState<FieldAgent[]>([]);
  const [query, setQuery] = useState("");
  const [page, setPage] = useState(1);
  const [addOpen, setAddOpen] = useState(false);
  const [edit, setEdit] = useState<FieldAgent | null>(null);
  ...
  const openEdit = (a: FieldAgent) => {
    setEdit(a);
    setForm({ name: a.name, phone: a.phone, zone: a.zone, status: a.status });
  };
  ...
  const filtered = items.filter((a) => !q || a.name.toLowerCase().includes(q) || a.zone.toLowerCase().includes(q));
  ...
  return (
    <>
      <div className="table-card">
        <div className="table-header">
          ...
          <div className="table-actions">
            <div className="search-box" style={{ padding: "5px 10px" }}>
              <span>🔍</span>
              <input type="text" placeholder="ค้นหาชื่อ / พื้นที่..." value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} style={{ width: 200 }} />
            </div>
        ...
        <table>
          <thead>
            <tr><th>รหัส</th><th>ชื่อ-นามสกุล</th><th>โทรศัพท์</th><th>พื้นที่</th><th>สถานะ</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {rows.map((a) => {
